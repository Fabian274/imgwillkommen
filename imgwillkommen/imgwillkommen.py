...
class ImgWillkommen(commands.Cog):
	...
	async def _create_welcome(J,member,test_member_number:int=_B):
		...
		except Exception as x:print('imgwillkommen cog: {e}');H=p
	...
	@commands.guild_only()
	@commands.group()
	async def imgwillkommen(self,ctx):"Configuration options for the welcome image.\n        You must have server administrator perms or the bot's Admin or Mod role to use imgwillkommen commands."
	@imgwillkommen.command(name='border')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_border(self,ctx,bordercolor=_B):
	...
	@imgwillkommen.command(name='channel')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_channel(self,ctx,channel:discord.TextChannel):
	...
	@imgwillkommen.command(name=_U)
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_clear(self,ctx):'Set the background to transparent.';await self.config.guild(ctx.guild).BACKGROUND.set(f"{self.imgpath}/transparent.png");await ctx.send('Welcome image background is now transparent.')
	@imgwillkommen.command(name='outline')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_outline(self,ctx,outline=_B):
	...
	@imgwillkommen.command(name='preview')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_preview(self,ctx,member:discord.Member=_B,number:int=_B):
	...
	@imgwillkommen.command(name='size')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_profilesize(self,ctx,profilesize:int):
	...
	@imgwillkommen.command(name='text')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_text(self,ctx,textcolor:str, servercolor:str):
	...
	@imgwillkommen.command(name='toggle')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_toggle(self,ctx):
	...
	@imgwillkommen.command(name='upload')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_upload(self,ctx,default=_B):
	...
	@imgwillkommen.group(name='bonus')
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_bonus(self,ctx):'Toggle display of additional text welcome messages when a user joins the server.'
	@imgwillkommen_bonus.command(name='user')
	@checks.mod_or_permissions(administrator=_A)
	...
	@imgwillkommen_bonus.command(name='warn')
	@checks.mod_or_permissions(administrator=_A)
	...
	@imgwillkommen.group(name='font',invoke_without_command=_A)
	@checks.mod_or_permissions(administrator=_A)
	async def imgwillkommen_font(self,ctx):
	...
	@staticmethod
	async def _font_file(fontpath):'Return font names for the imgwillkommen group command.';D='.';A=fontpath;B=A.split('/')[-1].split(D)[0];C=A.rsplit(D,1)[1];return B,C
	@imgwillkommen_font.command(name='reset')
	@checks.mod_or_permissions(administrator=_A)
	...
	@imgwillkommen_font.command(name='list')
	@checks.mod_or_permissions(administrator=_A)
	async def fontg_list(self,ctx):
		...
		G=sorted(F+D)
		if len(C)and len(B)==0:return await A.bot.send_message(channel,'No fonts found. Place fonts in your <data directory>/ImgWillkommen/fonts/.')
		E='[Current fonts]\n'
		for H in G:E+=f"{H}\n"
		for I in pagify(E,delims=['\n']):await ctx.send(box(I,lang='ini'))
	@imgwillkommen_font.command(name='name')
	@checks.mod_or_permissions(administrator=_A)
	async def fontg_name(self,ctx,font_name:str,size:int=_B):
		'Change the name text font.\n        e.g. [p]imgwillkommen font name "UniSansHeavy.otf"\n        ';D=font_name;C=self;B=size;A=ctx;E=f"{C.datapath}/fonts/"
	...
	@imgwillkommen_font.command(name='server')
	@checks.mod_or_permissions(administrator=_A)
	...
	@imgwillkommen_font.command(name='welcome')
	@checks.mod_or_permissions(administrator=_A)
	...
	@imgwillkommen.command(name='version',hidden=_A)
	async def imgwillkommenset_version(self,ctx):'Displays the imgwillkommen version.';A=base64.b64decode(b'VGhpcyBjb2cgaXMgb25seSBmb3IgUmVkLURpc2NvcmRCb3QgdjMu');A=A.decode('utf-8');await ctx.send(f"imgwillkommen version {self.version}.\n{A}")
